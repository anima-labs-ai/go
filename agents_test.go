...
func TestAgentsService_Get(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/v1/agents/agent_abc", func(w http.ResponseWriter, r *http.Request) {
...
func TestAgentsService_RotateKey(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/v1/agents/agent_abc/rotate-key", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("expected POST, got %s", r.Method)
		}
